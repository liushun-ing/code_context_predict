package com.example.contextcodepredict.myjstereocode.rules;
...
public class TypeStereotypeRules {
  ...
  public TypeStereotypeRules(PsiClass type, double methodsMean, double methodsStdDev) throws NullPointerException {
    this();
    if (type == null) {
  ...
  protected boolean checkForPool() {
    ...
    for (int var4 = 0; var4 < var5; ++var4) {
      PsiField field = var6[var4];
      if (field.getModifierList() != null && field.getModifierList().hasExplicitModifier(PsiModifier.FINAL) && field.getModifierList().hasExplicitModifier(PsiModifier.STATIC)) {
        ++staticFinalFields;
      }
    }
